...
export function RewardsStrip() {
  ...
  return (
    <section className="mt-3 rounded-xl border border-white/10 bg-[#0d1318] px-4 py-3">
      <div className="grid gap-3 xl:grid-cols-[1.1fr_2fr_auto] xl:items-center">
        <div>
          <p className="text-2xl font-black tracking-[0.16em] text-lime-300">ELEVATE REWARDS</p>
          <p className="text-xs tracking-[0.11em] text-slate-300">MORE POINTS. MORE PERKS. MORE ELEVATION.</p>
        </div>
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {perks.map((perk) => (
            <div key={perk.title} className="rounded-lg border border-white/10 bg-white/5 px-2.5 py-2">
        ...
        <div className="xl:text-right">
          <button
            onClick={openRewards}
            className="rounded-lg bg-lime-300 px-5 py-2.5 text-xs font-black tracking-[0.12em] text-black"
          >
            JOIN FOR FREE
          </button>
